...
class CleanerUI:
    def __init__(self, username, password, position):
        ...
        self.window_cleaner = tkinter.Tk()
        # 标题
        self.window_cleaner.title('Welcome to "Freedom Restaurant"')
        # 设置窗口不可变
        self.window_cleaner.resizable(False, False)
        # 屏幕宽和高
        screen_width_cleaner = self.window_cleaner.winfo_screenwidth()
        screen_height_cleaner = self.window_cleaner.winfo_screenheight()
        # 屏幕大小和位置
        self.window_cleaner.geometry(
            '%dx%d+%d+%d' % (1000, 700, (screen_width_cleaner - 1000) / 2, (screen_height_cleaner - 700) / 2 - 30))
        """
        show personal information
        """
        self.frame_personal_info = tkinter.Frame(self.window_cleaner, bg="#F8F8FF",
        ...
        img = Image.open(os.path.realpath("../resources/Qbanxiaoxu.jpg"))
        img.resize((150, 150))
        ...
        操作容器
        """
        self.frame_operation = tkinter.Frame(self.window_cleaner, bg="#DCDCDC",
                                             width=150, height=550)
        self.frame_operation.grid(columnspan=15, rowspan=55)

        btn_r = tkinter.Button(self.frame_operation, text="Thank you", command=self.thank_you)
        ...
        # 操作结果容器
        self.frame_result = tkinter.Frame(self.window_cleaner, bg='#F0F8FF', width=850, height=550)
        self.frame_result.grid(column=15, row=15, columnspan=85, rowspan=55)

        self.window_cleaner.mainloop()

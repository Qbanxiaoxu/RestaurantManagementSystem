...
from sample.gui.cashierui import CashierOperation
...
class CashierUI:
    def __init__(self, username, password, position):
        ...
        self.window_cashier = tkinter.Tk()
        # 标题
        self.window_cashier.title('Welcome to "Freedom Restaurant"')
        # 设置窗口不可变
        self.window_cashier.resizable(False, False)
        # 屏幕宽和高
        screen_width_cashier = self.window_cashier.winfo_screenwidth()
        screen_height_cashier = self.window_cashier.winfo_screenheight()
        # 屏幕大小和位置
        self.window_cashier.geometry(
            '%dx%d+%d+%d' % (1000, 700, (screen_width_cashier - 1000) / 2, (screen_height_cashier - 700) / 2 - 30))
        """
        show personal information
        """
        self.frame_personal_info = tkinter.Frame(self.window_cashier, bg="#F8F8FF",
        ...
        img = Image.open(os.path.realpath("../resources/cashier.png"))
        img.resize((150, 150))
        ...
        操作容器
        """
        self.frame_operation = tkinter.Frame(self.window_cashier, bg="#DCDCDC",
        ...
        # 操作结果容器
        self.frame_result = tkinter.Frame(self.window_cashier, bg='#F0F8FF', width=850, height=550)
        self.frame_result.grid(column=15, row=15, columnspan=85, rowspan=55)

        self.window_cashier.mainloop()

    def menu_info(self):
        CashierOperation(self.frame_result, 'menu')

    def restaurant_table_info(self):
        CashierOperation(self.frame_result, 'restaurant_table')

    def employee_info(self):
        CashierOperation(self.frame_result, 'employee')

    def customer_info(self):
        CashierOperation(self.frame_result, 'customer')

...
class ManagerUI:
    def __init__(self, username, password, position):
        ...
        self.window_manager = tkinter.Tk()
        # 标题
        self.window_manager.title('Welcome to "Freedom Restaurant"')
        # 设置窗口不可变
        self.window_manager.resizable(False, False)
        # 屏幕宽和高
        screen_width_manager = self.window_manager.winfo_screenwidth()
        screen_height_manager = self.window_manager.winfo_screenheight()
        # 屏幕大小和位置
        self.window_manager.geometry(
            '%dx%d+%d+%d' % (1000, 700, (screen_width_manager - 1000) / 2, (screen_height_manager - 700) / 2 - 30))
        """
        show personal information
        """
        self.frame_personal_info = tkinter.Frame(self.window_manager, bg="#F8F8FF",
        ...
        img = Image.open(os.path.realpath("../resources/manager.jpg"))
        img.resize((150, 150))
        ...
        操作容器
        """
        self.frame_operation = tkinter.Frame(self.window_manager, bg="#DCDCDC",
                                             width=150, height=550)
        self.frame_operation.grid(columnspan=15, rowspan=55)

        btn_r = tkinter.Button(self.frame_operation, text="restaurants information", command=self.restaurant_info)
        ...
        # 操作结果容器
        self.frame_result = tkinter.Frame(self.window_manager, bg='#F0F8FF', width=850, height=550)
